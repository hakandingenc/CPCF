...
(redex-check
 ...
 (let ([i 50])
   ...
   (define sl (apply-reduction-relation*/n CPCF-O-red (term (decorate "b/o" p)) i))
   (when (not (= (length fl) (length sl)))
     (traces CPCF-I-red (term (decorate "b" p)))
     (traces CPCF-O-red (term (decorate "b/o" p)))
     (error))
   (andmap
    (λ (t1 t2)
      (unless (alpha-equivalent? CPCF-I t1 (term (undecorate ,t2) #:lang CPCF-O))
...
#|
(for/and ([O-terms (in-list (map (λ (t) (term undecorate t)) (apply-reduction-relation*/n CPCF-O-red (term e) i)))]
